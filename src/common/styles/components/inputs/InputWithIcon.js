...
`;
const InputWithIcon = ({isError, autoFocus, onKeyDown, ref, maxLength, minLength, isFocused, value,name, onClick, isWarning, unitBgColor, onChange, placeholder, inputText, onBlur, onFocus, disabled,defaultValue,type, id}) => {
    const handleFocus =(e)=>{
     ...
     const handleChane=()=>{
      onChange(
        type === "text"
          ? { target: { name: name, value: "",id:id } }
          : { target: { name: name, value: "0",id:id } }
      );
     };
  return (
    <>
    <StyledInputWrapper isError={isError} isFocused={isFocused} disabled={disabled} id={id} isWarning={isWarning}>
        <StyledInput onKeyDown={onKeyDown} id={id} type={type} value={value} onFocus={handleFocus} ref={ref} autoFocus={autoFocus}
        onBlur={handleBlur} onChange={onChange} placeholder={placeholder} maxLength={maxLength} minLength={minLength}
